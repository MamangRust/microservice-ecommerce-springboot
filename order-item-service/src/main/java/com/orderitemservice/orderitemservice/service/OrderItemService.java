...
public class OrderItemService {
    ...
    public List<OrderItem> getByOrderId(Long orderId) { return repository.findByOrderId(orderId); }

    public OrderItem getById(Long id) { return repository.findById(id).orElseThrow(() -> new RuntimeException("Order item not found")); }

    public OrderItem create(OrderItemRequest req) { return repository.save(mapper.toEntity(req)); }

    public OrderItem update(Long id, OrderItemRequest req) {
        OrderItem e = getById(id);
        e.setQuantity(req.quantity()); e.setPrice(req.price());
