...
The manifests under `base/` describe the active Java Quarkus services and their
...
2. A `ClusterSecretStore` named `ecommerce-secrets` must exist and provide these
   remote keys:
   `ecommerce/DB_USER`, `ecommerce/DB_PASS`, `POSTGRES_USER`,
...
```text
ghcr.io/<owner>/quarkus-ecommerce/<service>:<git-sha>
ghcr.io/<owner>/quarkus-ecommerce/<service>:latest
```
...
immutable tag per release. Pin the current commit SHA to every `newTag` in
`overlays/production/kustomization.yaml` with the helper script (a CI job can
call this after each build):

```bash
./deployments/kubernetes/promote-image.sh "$(git rev-parse HEAD)"
```

ArgoCD then rolls out exactly the artifacts that SHA produced. To verify a
pinned tag before rollout, confirm it exists in GHCR and perform a server-side
