...
public class OrderService {
        ...
        private void writeOrderOutbox(Order order) {
                try {
                        Map<String, Object> payload = Map.of(
                                "eventId", UUID.randomUUID().toString(),
                                "orderId", order.getId().toString(),
                                "userId", order.getUserId() == null ? "" : order.getUserId().toString(),
                                "productId", order.getProductId() == null ? "" : order.getProductId().toString(),
                                "quantity", order.getQuantity() == null ? 0 : order.getQuantity(),
                                "status", order.getPaymentStatus() == null ? "PENDING" : order.getPaymentStatus().name()
                        );
                        EventEnvelope<Map<String, Object>> envelope =
                                EventEnvelope.withDefaults(payload, "order.created", "order");
                        ...
                        log.info("Order outbox written for order {}", order.getId());
                } catch (Exception e) {
                        log.error("Failed to write order outbox: {}", e.getMessage());
                }
        }
}