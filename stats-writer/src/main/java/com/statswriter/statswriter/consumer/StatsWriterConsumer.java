...
public class StatsWriterConsumer {
    ...
    @KafkaListener(topics = "stats.ecommerce.order.event", groupId = "stats-writer")
    ...
    @KafkaListener(topics = "stats.ecommerce.transaction.event", groupId = "stats-writer")
        ...
        try {
            for (Map<String, Object> row : batch) {
                clickhouseJdbc.update(
                    "INSERT INTO ecommerce_stats.order_daily (event_id, event_version, occurred_at, " +
                    "order_id, product_id, user_id, quantity, status) " +
                    "VALUES (?, 1, now(), ?, ?, ?, ?, ?)",
                    row.get("eventId"), row.get("orderId"), row.get("productId"),
                    row.get("userId"), row.get("quantity"), row.get("status"));
            }
    ...
    public void flushTransactionBuffer() {
        ...
        try {
            for (Map<String, Object> row : batch) {
                clickhouseJdbc.update(
                    "INSERT INTO ecommerce_stats.transaction_daily (event_id, event_version, occurred_at, " +
